...
export async function loadRegistry(root,sourceRevision=null){
  ...
  assert(site.analytics.productionHostname==='tools.engroveaudio.com','analytics production hostname mismatch');
  assert(/^G-[A-Z0-9]+$/.test(site.analytics.googleAnalyticsMeasurementId),'Google Analytics measurement ID invalid');
  assert(/^[a-z0-9]+$/.test(site.analytics.microsoftClarityProjectId),'Microsoft Clarity project ID invalid');
