...
export async function copyRuntime(from,to){
  for(const entry of await fs.readdir(from,{withFileTypes:true})){
    if(SKIP.has(entry.name)||EXT.has(path.extname(entry.name)))continue;
...
function structuredToolSlugs(value){
  assert(Array.isArray(value.tools),'structured tool list missing');
  return value.tools.map((tool)=>tool.slug);
}

export async function checkOutput(root,out,registry){
  const required=[
    'index.html','tools.json','robots.txt','sitemap.xml','sitemap.json','sitemap.txt',
    'llms.txt','llms-full.txt','for-agents.md','for-agents/index.html','ai.json',
    'agent-capabilities.json','public-info.json','.well-known/engrove-tools.json',
    '.well-known/agent-skills/index.json','.well-known/agent-skills/engrove-tools/SKILL.md','_headers',
  ];
  ...
  for(const file of ['tools.json','ai.json','agent-capabilities.json','public-info.json','.well-known/engrove-tools.json']){
    const value=JSON.parse(await fs.readFile(path.join(out,file),'utf8'));
    exactSet(structuredToolSlugs(value),expected,file);
  }
  ...
  for(const tool of registry.publicTools){
    ...
    assert(scripts.length===1,`JSON-LD missing: ${tool.slug}`);
    const graph=JSON.parse(scripts[0][1]);
    const faqNode=graph['@graph']?.find((node)=>node['@type']==='FAQPage');
    assert(Boolean(faqNode)===Boolean(tool.faq.length),`FAQ JSON-LD polarity failure: ${tool.slug}`);
    for(const faq of tool.faq)assert(page.includes(faq.question)&&page.includes(faq.answer),`FAQ parity failure: ${tool.slug}`);
    for(const related of tool.relatedTools){
      const target=registry.tools.find((candidate)=>candidate.slug===related);
      assert(page.includes(`href="${target.canonicalPath}"`),`related-tool link missing: ${tool.slug}->${related}`);
    }
    const launchIndex=page.indexOf('<h2>Launch tool</h2>');
    const evidenceIndex=page.indexOf('<h2>Evidence and source links</h2>');
    assert(launchIndex>evidenceIndex&&launchIndex>0,`launch section ordering failure: ${tool.slug}`);
    assert(await exists(path.join(out,'tools',tool.slug,'app','index.html')),`runtime entry missing: ${tool.slug}`);
    assert(await exists(path.join(out,'tools',tool.slug,'tool.ai.json')),`tool.ai.json missing: ${tool.slug}`);
    assert(await exists(path.join(out,'tools',tool.slug,'tool.md')),`tool.md missing: ${tool.slug}`);
  }

  const sitemap=JSON.parse(await fs.readFile(path.join(out,'sitemap.json'),'utf8'));
  const urls=[`${registry.site.canonicalOrigin}/`,...registry.publicTools.map((tool)=>tool.canonicalUrl)].sort();
  assert(JSON.stringify([...sitemap.urls].sort())===JSON.stringify(urls),'sitemap parity failure');

  const robots=await fs.readFile(path.join(out,'robots.txt'),'utf8');
  assert(robots.includes('Content-Signal: search=yes, ai-input=yes, ai-train=no'),'robots policy missing');
  const headers=await fs.readFile(path.join(out,'_headers'),'utf8');
  assert(headers.includes('/tools/*/app/*')&&headers.includes('X-Robots-Tag: noindex, nofollow'),'runtime noindex policy missing');

  const full=await fs.readFile(path.join(out,'llms-full.txt'),'utf8');
  for(const tool of registry.publicTools){
    for(const text of [tool.name,tool.description,...tool.whenToUse,...tool.whenNotToUse,...tool.limitations,...tool.claims.mayClaim,...tool.claims.negativeClaimBoundaries,...tool.claims.mustNotClaim])assert(full.includes(text),`llms-full parity failure: ${tool.slug}`);
  }

  for(const file of await listFiles(out)){
    const relative=path.relative(out,file);
    const stat=await fs.stat(file);
    assert(stat.size<=25*1024*1024,`asset too large: ${relative}`);
    assert(!EXT.has(path.extname(file))&&!/(^|[\\/])(node_modules|test|tests|src)([\\/]|$)/.test(relative),`forbidden output: ${relative}`);
  }

  const middleware=await fs.readFile(path.join(root,'functions','_middleware.ts'),'utf8');
  assert(middleware.includes("hostname.endsWith('.pages.dev')")&&middleware.includes('noindex, nofollow')&&middleware.includes('text/markdown'),'preview policy missing');
