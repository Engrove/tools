...
export function validatePackage(pkg, schemas) {
  ...
  for (const { record: file, index } of validFiles) {
    const p = file.path;
    if (p === 'manifest.json') {
      errors.push(fail('MANIFEST_SELF_INVENTORY', 'manifest.json must not appear in manifest.files[]', `manifest.files[${index}].path`));
      continue;
    }
  ...
  for (const path of Object.keys(entries)) if (path !== 'manifest.json' && !seen.has(path)) errors.push(fail('UNDECLARED_FILE', `entry absent from manifest: ${path}`, `entries.${path}`));
  ...
  const manifestByPath = new Map(validFiles.filter(({ record }) => record.path !== 'manifest.json').map(({ record }) => [record.path, record]));
  for (const { record: asset, index } of assetRecords) {
