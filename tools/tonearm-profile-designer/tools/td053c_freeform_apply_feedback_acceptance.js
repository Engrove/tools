/**
 * AI-CODING NOTE:
 * Responsibility: Preserve the imported TD053F td053c_freeform_apply_feedback_acceptance.js behavior as a cohesive legacy module.
...
// TD053C apply feedback/status visibility acceptance.
...
check('status codes are explicit and source-wired', () => {
  ['VALIDATION_PASS','VALIDATION_REJECTED','APPLY_PASS','APPLY_BLOCKED','APPLY_FAILED','PARSING','VALIDATING'].forEach(code => {
    assert(runtime.includes(code), 'missing status code ' + code);
  });
  assert(runtime.includes('payload applicerades inte'), 'missing not-applied feedback wording');
  assert(runtime.includes('firstErrorPath'), 'missing first error path extraction');
  assert(runtime.includes('firstErrorConstraint'), 'missing first error constraint extraction');
});

check('invalid JSON path has parse error feedback', () => {
  assert(runtime.includes('JSON parse error'), 'missing parse error feedback text');
  assert(runtime.includes('APPLY_FAILED'), 'missing APPLY_FAILED parse path');
});

check('schema rejection writes validation panel with path and constraint', () => {
  assert(html.includes('id="freeformWorkbenchValidationPanel"'), 'missing validation panel');
  assert(runtime.includes('formatValidationErrors'), 'missing validation formatter');
  assert(runtime.includes('errorCount'), 'missing error count output');
  assert(runtime.includes('firstPath'), 'missing first path output');
  assert(runtime.includes('firstConstraint'), 'missing first constraint output');
});

check('valid payload still passes runtime validator', () => {
  const payload = makeUserPayload();
  const result = globalThis.FreeformLoftKernel.validateResponse(payload);
  assert(result.ok, 'valid TD053C payload rejected: ' + result.errors.join('; '));
});
