/**
 * AI-CODING NOTE:
 * Responsibility: Preserve the imported TD053F td053c_freeform_designintent_no_maxlength_acceptance.js behavior as a cohesive legacy module.
...
// TD053C arbitrary text maxLength removal acceptance.
...
loadKernel();
const schema = JSON.parse(fs.readFileSync(path.join(ROOT, 'tonearm_designer_ai_freeform_loft.schema.json'), 'utf8'));

check('arbitrary text fields have no maxLength', () => {
  assert(!('maxLength' in schema.$defs.DesignIntent), 'DesignIntent still has maxLength');
  assert(!('maxLength' in schema.$defs.AiEstimates.properties.reasoningSummary), 'aiEstimates.reasoningSummary still has maxLength');
  assert(!('maxLength' in schema.$defs.Assumptions.items), 'assumptions[] still has maxLength');
  assert(!('maxLength' in schema.$defs.UnsupportedAttributes.items), 'unsupportedAttributes[] still has maxLength');
});

check('long design text validates at runtime', () => {
  const payload = makeUserPayload();
  payload.designIntent += ' '.repeat(1) + makeLongText('extra design intent', 16);
  payload.aiEstimates.reasoningSummary += ' ' + makeLongText('extra reasoning', 18);
  payload.assumptions.push(makeLongText('long assumption field', 12));
  payload.unsupportedAttributes.push(makeLongText('long unsupported attribute field', 8));
  const result = globalThis.FreeformLoftKernel.validateResponse(payload);
  assert(result.ok, 'runtime rejected long text payload: ' + result.errors.join('; '));
});

check('technical identifiers keep justified maxLength only', () => {
  assert(schema.$defs.CenterlinePointPatch.properties.id.maxLength === 64, 'id maxLength changed unexpectedly');
  assert(schema.$defs.FreeformLoftResponse.properties.name.maxLength === 80, 'name identifier maxLength missing');
});
