/**
 * AI-CODING NOTE:
 * Responsibility: Preserve the imported TD053F fas16_1_gemini3_export_acceptance.js behavior as a cohesive legacy module.
...
let code = '';
['js/config.js', 'js/math.js', 'js/geometry.js'].forEach(rel => { code += fs.readFileSync(path.join(ROOT, rel), 'utf8') + '\n'; });
code += `
...
updateGeometryCache();
const validation = validateExportGeometry(state.exportType, state);
console.log(JSON.stringify({
  schema: 'tonearm-designer-fas16-1-gemini3-export-acceptance-v1',
  status: validation && validation.isValid ? 'PASS' : 'FAIL',
  exportType: state.exportType,
  rearMode: state.rearMode,
  inputRearFineTrimScrewMassEquivalent: ${JSON.stringify(session.inputs.rearFineTrimScrewMassEquivalent)},
  sanitizedRearFineTrimScrewMassEquivalent: state.rearFineTrimScrewMassEquivalent,
  validation
}, null, 2));
if (!validation || !validation.isValid) throw new Error(validation && validation.errorMsg ? validation.errorMsg : 'Gemini_3 export validation failed');
if (!(validation.metrics && validation.metrics.rearCapDot >= 0.9)) throw new Error('Rear cap tangent-normal validation did not pass');
if (state.rearFineTrimScrewMassEquivalent < 0) throw new Error('Negative mass was not sanitized');
`;
