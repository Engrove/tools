/**
 * AI-CODING NOTE:
 * Responsibility: Preserve the imported TD053F td053d_freeform_mode_authority_acceptance.js behavior as a cohesive legacy module.
...
const M = globalThis.FreeformModeAudit;

check('apply Freeform response sets geometryMode=freeform in kernel and runtime state', () => {
  const payload = makeHybridPayload();
  const result = K.applyResponse(K.defaultState('long_low_cobra_monocoque'), payload);
  assert(result.ok, 'applyResponse failed');
  assert(globalThis.state.geometryMode === 'freeform', 'kernel apply did not set global state geometryMode=freeform');
});

check('runtime freeform build exposes mode audit sources', () => {
  const s = { geometryMode: 'parametric', freeformLoft: K.defaultState('long_low_cobra_monocoque') };
  R.setGeometryMode('freeform', s);
  const go = R.buildFreeformGeometry(s, { stationCount: 18, segmentCount: 32 });
  assert(go.geometryModeAudit, 'missing geometryModeAudit');
  assert(go.geometryModeAudit.renderGeometrySource === 'freeformLoftKernel', 'wrong render source');
  assert(go.geometryModeAudit.exportGeometrySource === 'freeformLoftKernel', 'wrong export source');
  assert(go.geometryModeAudit.reportGeometrySource === 'freeformPhysicalAnalysis', 'wrong report source');
  assert(go.geometryModeAudit.modeMismatch === false, 'unexpected mode mismatch');
});

check('parametric mode with freeformLoft is BLOCKER mismatch', () => {
  const audit = M.auditGeometryMode({ geometryMode: 'parametric', freeformLoft: K.defaultState('long_low_cobra_monocoque') }, null, null, null);
  assert(audit.modeMismatch === true, 'mode mismatch not detected');
  assert(audit.status === 'BLOCKER', 'mode mismatch not BLOCKER');
});

check('session source includes geometryModeAudit and mismatch warning', () => {
  const session = fs.readFileSync(path.join(ROOT, 'js/session.js'), 'utf8');
  assert(session.includes('geometryModeAudit'), 'session does not persist geometryModeAudit');
  assert(session.includes('freeformModeMismatchWarning'), 'session load warning missing');
});
